    ...
    pub async fn draw_frame(&mut self, play_state: &Scene, slide_real_pc: f32, anim_real_pc: f32, ghost_state: &scene::Play, ghost_opacity: f32, anim_ghost_pc: f32) {
        ...
        match play_state {
            Scene::Play(play_state) => {
                let mut render_lev = RenderLev::begin(&mut self.texture_cache, play_state.field.map.borrow().w(), play_state.field.map.borrow().h());
                // Coords of first visible tile. Currently always 0,0.
                let (ox, oy) = (0, 0);
                let max_h = 5;
                for h in 0..max_h {
                    for (x, y, loc) in play_state.field.map.borrow().locs() {
                ...
                if draw_ghosts
                {
                    ...
                    for (x, y, loc) in ghost_state.field.map.borrow().locs() {
                        for ent in loc {
